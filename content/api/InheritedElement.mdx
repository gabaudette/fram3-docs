...
description: "live An element that propagates data down the element tree, accessible to descendants via GetInherited or FindInherited. When the parent rebuilds and provides a new inherited element of the same type, all descendants that declared a dependency via DependOnInherited are automatically scheduled for a rebuild -- but only when InheritedElement) returns true."
since: "2.0.0-beta.1"
status: "live"
