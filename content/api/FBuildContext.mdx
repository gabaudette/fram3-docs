...
Walks up the element tree looking for the nearest ancestor that is an FInheritedElement of type , registers this element as a dependent of that ancestor, and returns the inherited element. When the inherited element is later replaced and FInheritedElement) returns true, this element's subtree will be automatically scheduled for a rebuild. Returns null when no matching ancestor exists.
...
Walks up the element tree looking for the nearest ancestor that is an FInheritedElement of type , registers this element as a dependent, and returns the inherited element. Throws when no matching ancestor exists.
